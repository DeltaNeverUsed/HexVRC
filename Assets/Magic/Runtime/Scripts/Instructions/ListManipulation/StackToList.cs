...
namespace BefuddledLabs.Magic.Instructions.ListManipulation
{
    public static class StackToList
    {
        ...
        public const string Description = "Remove num elements from the stack, then add them to a list at the top of the stack.";
        ...
        public static ExecutionState Execute(ExecutionInfo info, float length)
        {
            ...
            if (info.Stack.Count < size)
                return ExecutionState.Err($"Not enough elements on the stack. Needed {size}, but only {info.Stack.Count} available.");
            ...
            info.Stack.Push(StackItem.CreateStackItem(array));
